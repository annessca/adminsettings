...
function og_custom_add_menu_options() {
	add_theme_page('Ogun Labs Profile', 'Ogun Labs Profile', 'edit_theme_options', 'ogcustom-settings', 'og_custom_homepage_admin_options');
}
...
function og_custom_homepage_admin_options() {
	...
	<!-- WP CSS predefined styling for the Admin Panel viewing -->
		<div class="wrap">
			...
			<h2><?php _e( 'Ogun Labs\' Profile', 'og_custom' ); ?></h2>
...
function  og_custom_image_settings_setup() {
	...
	// Add form sections
	add_settings_section('site_logo', __( 'Manage Image upload', 'og_custom' ), 'og_custom_settings_description', 'og_custom');
	
	// Add Fields to Section	
	add_settings_field('og_custom_logo',  __( 'Site Logo', 'og_custom' ), 'og_custom_set_logo', 'og_custom', 'site_logo'); 
    add_settings_field('og_custom_logo_preview',  __( 'Site Logo Preview', 'og_custom' ), 'og_custom_logo_preview', 'og_custom', 'site_logo');
...
function og_custom_settings_description() {
	?>
		<p><?php _e( 'Update Site Logo from Theme Settings', 'og_custom' ); ?></p>
	<?php
}
...
function og_custom_set_logo() {
	...
	?>
		...
		<?php endif; ?>
		<span class="description"><?php _e('Upload Site Logo Image.', 'og_custom' ); ?></span>
	<?php
}
...
function replace_thickbox_text($translated_text, $text, $domain) {
	if ('Insert into Post' == $text) {
		...
		if ( $referer != '' ) {
			return __('Confirm this Image as Logo!', $domain );
		}
	}
